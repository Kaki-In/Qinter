...
class DelayHandler(object):
    def __init__(self, timeout, function):
        self._func = function
        self.endTime = time.monotonic()+timeout/1000
        
    verify = lambda self,monotonic:monotonic>self.endTime
    
    execute = lambda self:self._func()
...
class View(object):
    ...
    def add_delay(self, delayHandler):
         self._delays.append(delayHandler)
    ...
    def __setattr__(self, name, value):
        ...
        '''
        if name in ("view_type","view_id","_events", "_delays"):
            object.__setattr__(self,name,value)
        else:
            #sys.stderr.write("calling sl4a to set name:"+str(name)+" value:"+value+"\n")
            return FullScreenWrapper2App.set_property_value(self.id, name, str(value))
    ...
    def __getattr__(self, name):
        ...
        if name in ("view_type","view_id","_events", "_delays"):return object.__getattr__(self,name)
...
class Layout(object):
    ...
    def add_delay(self, delayHandler):
        self.views[self.uid].add_delay(delayHandler)
            ...
            while(True):
                ...
                if len(evt.result):
                    ...
                    try:
                        ...
                        eventdata['data']={"id":eventdata["name"], "extra":eventdata["data"]}
                        ...
                        for eventname in iter(view._events):
                            ...
                            if event.match_event_data(eventdata):
                                if event.event_handler_fn != None:
                                    ...
                                    break
                    if eventdata["data"]=={"id":"screen", "extra" : "destroy"}:
                        cls.close_layout()
                monot=time.monotonic() 
                for viewname in curlayout.views:
                    view = curlayout.views[viewname]
                    for delay in view._delays.copy():
                        if delay.verify(monot):
                            #sys.stderr.write("Executing a valable delay\n")
                            delay.execute()
                            view._delays.remove(delay)
        except:raise
        finally:
            try:curlayout.on_close()
            except:pass
            cls.close_layout()
